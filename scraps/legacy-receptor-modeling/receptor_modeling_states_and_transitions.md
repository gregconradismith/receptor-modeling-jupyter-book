...
One reason for using symbolic variables is that we can produce symbolic expressions for important quantities using the [module for graphs and digraphs](https://doc.sagemath.org/html/en/reference/graphs/index.html) available in `SageMath`. For example, the weighted adjacency matrix for `G` is 
